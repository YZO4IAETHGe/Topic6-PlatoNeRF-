...
def train():
    ...
    # FOR TESTING ONLY
    args.N_samples = 512 #1024
    batch_size = 1024

    output_dir = os.path.join(args.output_dir, "depth_final_test")
    ...
    lookat = np.array([0.5,-1,2])
    poses = look_at(np.array([-0.5,0,2.5]), lookat)

    Nframes = 40
    render_poses = []
    poses = np.identity(4)
    for i in range(Nframes):
        rpose = deepcopy(poses)
