...
def create_nerf(args):
    ...
    skips = [4]
    if SIREN:
        model = SIREN_NET(D=args.netdepth, W=args.netwidth,
    ...
    if args.N_importance > 0:
        if SIREN:
            model_fine = SIREN_NET(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
...
def train():
    ...
    # Initialize loss history for saving to .npy
    loss_history = []
    loss_file = os.path.join(basedir, expname, 'loss_history.npy')
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_print==0:
            ...
            np.save(loss_file, np.array(loss_history))
